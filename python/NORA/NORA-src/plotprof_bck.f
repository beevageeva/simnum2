...
C      * Dibuja en cualquier device (LNO3, TEK4010, VERSATEC,...)
C	Si device=terminal (TEK4010) cambia lw a 1 (en laser es 2)
c
c
...
C
C				JIGS  GRON  MARZ-89
C        UTILIZA PGPLOT (FIGARO)   GRPSHR/LIB
...
1	READ(1,*,END=2) (A(J,I),J=1,30)
	A(4,I)=PIXEL*A(4,I)                   !SEMIEJE MAYOR EN ARCSECS
	I=I+1
	GOTO 1
2	CONTINUE
	N=I-1
...
C	ABRIMOS DEVICE
	Print*,'Device: '
	Read(*,'(a)') Cacha
	CALL PGBEGIN(0,Cacha,2,2)

	Call Pgvport (0.1,0.95,0.1,0.95)

C       INQUIRE DEVICE
c	CALL PGQINF('TYPE',CACHA,LONGI)
C	IF (Cacha.eq.'/retro'.or.Cacha.eq.'/RETRO') IANCH=1
C	IF (Cacha.eq.'827') IANCH=1
c	CACHA='/'//CACHA
...
C	LOG I -- R**.25
	...
	CALL PGLABEL('a\u1/4\d (arcsec)','Log I', title)
...
C	LOG I (MODA) -- R**.25 + LOG I -- R**.25
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('a\u1/4\d (arcsec)','Log I (moda+med) (counts)', title)
...
C	ABRIMOS DEVICE
CCC	CALL PGBEGIN(0,CACHA,2,1)
CCC	Call Pgvport (0.1,0.95,0.1,0.95)
CCC	CALL PGADVANCE
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('Log a (arcsec)','P.A. (degrees)', title)
...
C	elipticidad
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('Log a (arcsec)','Ellipticity', title)
	CALL PGPOINT(IN,X,Y,17)
	Call Pgadvance
...
CCC	CALL PGADVANCE
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('Log a (arcsec)','B1, A1', title)
	CALL PGPOINT(IN,X,Y,17)
...
C	B2 A2
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('Log a (arcsec)','B2, A2', title)
	CALL PGPOINT(IN,X,Y,17)
	call pgpoint(in,x,ybis,2)
C	ABRIMOS DEVICE
	Call Pgadvance
...
C	B3,B4, A3, A4  -- LOG R
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('Log a (arcsec)','B3, A3', title)
	CALL PGPOINT(IN,X,Y,17)
	call pgpoint(in,x,ybis,2)
	CALL PGADVANCE
C	B4
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('Log a (arcsec)','B4, A4', title)
	CALL PGPOINT(IN,X,Y,17)
	call pgpoint(in,x,ybis,2)
	Call Pgadvance
c
...
c		if (fase3.ge.pi/2.) fase3=fase3-pi/2.
		...
		z(1,kk)=amp3
	enddo
c	CALL MAXMIN(y,IN,1,1,YMAX,YMIN)
	YMIN=-90.0
	YMAX=90.0 
	XXMAX=ALOG10(XMAX)+.1
	XXMIN=ALOG10(XMIN)-0.1
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('Log a (arcsec)','Phase 3 theta', title)
	CALL PGPOINT(IN,X,Y,17)
	call pgadvance
	...
	call pgwindow (xxmin,xxmax,ymin,ymax)
	call pgbox ('SBCNT',0.0,0,'SBCNT',0.0,0)
	call pglabel('Log a (arcsec)','Amplitude 3 theta',title)
...
c	CALL MAXMIN(y,IN,1,YMAX,YMIN)
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('Log a (arcsec)','Phase 4 theta', title)
	CALL PGPOINT(IN,X,Y,17)
	call pgadvance
	call maxmin (z,in,1,1,ymax,ymin)
	ymin=ymin-0.05
	...
	call pgwindow (xxmin,xxmax,ymin,ymax)
	call pgbox ('SBCNT',0.0,0,'SBCNT',0.0,0)
	call pglabel('Log a (arcsec)','Amplitude 4 theta',title)
	call pgpoint(in,x,z,17)
	Call Pgadvance	
...
CCC	CALL PGADVANCE
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('Log a (arcsec)','XC (pix)', title)
	CALL PGPOINT(IN,X,Y,17)
...
C	YC
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('Log a (arcsec)','YC (pix)', title)
	CALL PGPOINT(IN,X,Y,17)
	Call Pgadvance
c	
c	DELTA R/R -- LOG R
...
CCC	CALL PGADVANCE
	...
	CALL PGWINDOW(XXMIN,XXMAX,YMIN,YMAX)
	CALL PGBOX('SBCNT',0.0,0,'SBCNT',0.0,0)
	CALL PGLABEL('Log a (arcsec)','\gD R/R', title)
	CALL PGPOINT(IN,X,Y,17)
