...
C************************************************************************
                ...
                SUBROUTINE ddens (gdev, coordmax, istart, iend,
     &				infostr, dmin, dmax, d0)
...
C-----------------------------------------------------------------------------
	...
	INTEGER nbin
	PARAMETER (nbin=40)
	REAL dist(nbin), logdist(nbin), dlogr, dmin(2), dmax(2),
     1       d0
     ...
     1		meanvR(nbin), meanvT(nbin), rmax
...
C---------------
        d0=d0*totm/float(iend-istart+1)
        print *,' back ground density : ',d0
        DO 30 i = 1, nbin
                rho(i) = rho(i) / 
     1               (4.e0 * PI * (dist(i)**3 - dist(i-1)**3) / 3.e0)
                rho(i)=rho(i)/d0-1.
		IF (rho(i) .LE. 0.) rho(i) = 1.e-10
 30     CONTINUE
 
C--------
C  Output 
C--------
	CALL densout(gdev, dist, inbin, rho, meanvR, dispr, Rm, FRM,
     &		totm, infostr, dmin, dmax)
