...
C*****************************************************************************
        ...
        SUBROUTINE sbrout2(gdev, dist, Ngen, Mr, surf, disp, 
     &              GRAVC, proy,infostr)
...
C---Label 
	   ...
	   CALL PGTEXT (0., 0.7,'Surf Brightness and Dispersion prof')
	   CALL PGTEXT (0., 0.6, infostr(1))
	   CALL PGTEXT (0., 0.5, infostr(2))
	   CALL PGSCH (1.0)
           if(proy .LT. 2) then
C---Second page: exp plot
	CALL PGASK(.TRUE.)
	CALL PGADVANCE
	CALL PGVPORT (0.2, 0.8, 0.4, 0.8)
	CALL PGWINDOW(dist(1), dist(nbin), -8., 1.)
	CALL PGBOX ('BCNST', 0.0, 0, 'BCNST', 0.0, 0)
	CALL PGLABEL ('r', 'log(sbr)', ' ')
	CALL PGPOINT (nbin, dist, logsurf, cross)
        else if(proy .GE. 2) then
