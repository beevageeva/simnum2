...
C*****************************************************************************
                ...
                SUBROUTINE rotc (gdev, nbods, istart, iend,  theta, phi,
     &				 PA, coordmax, swidth,
...
c        PARAMETER (nbin=50, nhalf = nbin/2,  maxinbin = 100000)
	...
	CHARACTER*(*) infostr(*)
     ...
     &       xnew, ynew, xlog, weight
        ...
        REAL vr(nbin), disp(nbin), err(nbin), m_smooth(nbin)
...
c            IF (in_bin(j) .GE. 6) THEN
            IF (in_bin(j) .GE. 6e4) THEN
c     ????????????
...
C               WRITE(6,*) 'rotv>> wt(k) = ', (wt(k), k=1, in_bin(j))
                    ...
                    a=MDIAN3 (vl, wt, in_bin(j))
c                vrad(j) = MDIAN3 (vl, wt, in_bin(j))
...
C------ Smooth radial velocities and dispersions
     ...
     &                 vsqr(j-2) + vsqr(j+2)
          ...
          IF(m_smooth(j) .LT. 1.e-7 .OR. in_bin(j) .EQ. 0) THEN
            vr(j)    = 100.
            disp(j)  = 100.
            err(j)   = 0.1
          ELSE
            vr(j)    = vr(j) / m_smooth(j)
            disp(j) = SQRT(disp(j) / m_smooth(j))
...
c           nbin=100
     ...
     &	     theta, phi, GRAVC, infostr)
            open(10,file='barra.dat',STATUS ='UNKNOWN')
            do 90 j=1,nbin
            write(10,*)coord(j),m_smooth(j)*pmass(1)
 90         continue
            close(10)
        END IF
